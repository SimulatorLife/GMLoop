...
- The existing cross-workspace integration test fixtures (`test/fixtures/**/*.gml`), the parser fixtures (`src/parser/test/**/*.gml`), the refactor fixtures (`src/refactor/test/fixtures/**/*.gml`), the formatter's format-expectation fixtures (`src/format/test/fixtures/**/*.gml`), and the linter's auto-fix fixtures (`src/lint/test/**/*.gml`) in this project are considered golden files and **MUST NEVER BE MODIFIED UNLESS EXPLICITLY GIVEN PERMISSION**. They capture parsing behavior and the desired formatting and/or lint auto-fixing for GML code and must be preserved byte-for-byte. NEVER update or change these files, certainly not to "fix" a test failure. You may add/modify `*.test.ts` tests or adjust the way tests are executed, but **NOT** `*.gml` text fixtures. If you encounter a test failure, the correct course of action is to investigate the underlying issue and fix it in the source code or test logic, not to change the expected output in the fixture. These fixture tests are run as part of the normal test suite, driven by the shared fixture-runner workspace `@gmloop/fixture-runner`
...
- Do **NOT** modify agent-specific files such as `AGENTS.md`, `GEMINI.md`, `.github/copilot-instructions.md`, `.gemini/**`, `.qwen/**`, `.aiderignore`, `.codex/**`, `.agents/**` under any circumstances, including discarding/reverting changes. If you see modifications to these files, always assume that these were made by a human developer and do **NOT** revert them unless the user explicitly asks you to do so.
- Before implementing a task, inspect the repository's available skills under `.agents/skills/**` and apply the skill or skills that best match the request. If generic agent defaults conflict with a repository skill, follow the repository skill.
...
- Do **NOT** add standalone Node scripts outside of `src/cli/src/commands/`. Expose new tooling through the CLI so helpers remain discoverable and consistent. For any single-use/debugging/investigation scripts, you can create a temporary file in the project's root `tmp` directory (e.g., `tmp/debug-script.js`).
- Do **NOT** edit the generated files in `src/parser/generated`; any and all changes will be overwritten when the file are re-generated. The generated code is tightly coupled to ANTLR’s runtime. Even a small edit could break assumptions about rule indices, token streams, or the visitor/listener APIs.
- Do **NOT** use dynamic/boundary-skirting import patterns such as `await import(...)`, dynamic `import()` expressions, or `require()` calls in TypeScript. All imports must be static and at the top level of the file, using standard ES module syntax. This ensures that all dependencies are clear, type-checked, and compatible with the ESM module system.
...
- The formatter should be opinionated and enforce a single opinionated strategy (for indentation, spacing, blank lines, etc.) – avoid adding overly-configurable options that give users too many choices or lead to inconsistent formatting. For instance, instead of multiple options to control line-length formatting (such as `printWidth`, `wrapThreshold`, `maxLineLength`, etc.), there should be a single `printWidth` option that governs line length. Excessive configurability leads to maintenance burden and unpredictable output.
- **ONLY** the `format` workspace may depend on `prettier` and related formatting packages. All other workspaces must remain free of formatting dependencies. If they require formatting-related functionality, it must be moved into and exposed by the `format` workspace (only the project's root `package.json` should have Prettier as a `devDependency` for formatting of *this* codebase).
...
- Respect the formatter/linter split contract: semantic/content rewrites belong in `@gmloop/lint` (or refactor transactions), while `@gmloop/format` remains formatter-only and layout-focused.
...
- Use named scopes for all inter-workspace imports, always referencing workspaces by their declared name rather than using relative paths, and ensure that each workspace re-exports its public API at the top level so consumers import only from the workspace root (e.g., use `@gmloop/core` instead of deep paths like `"../../../src/core/src/ast/comments.js"`); this rule also applies in `package.json`, where inter-workspace dependencies must always be listed by workspace name rather than filesystem paths.
...
- Do not embed source code, HTML, CSS, or other standalone assets inside string literals or template strings. Avoid functions that “render” entire scripts/styles/documents as strings; instead, place the content in proper standalone files such as `.ts`, `.html`, or `.css`, and load/import them normally. Only use string-generated code where absolutely necessary for unavoidable scenarios with a clear justification.
...
## Workspace Ownership Boundaries (Parser / Core / Formatter)
>This section defines clear ownership boundaries between the parser, core, and format workspaces to ensure a clean architecture with well-defined responsibilities.
...
The `@gmloop/core` workspace *should* own:
...
The `@gmloop/core` workspace should **not** own:
- ❌ Parsing logic
- ❌ Formatting rules
- ❌ Printing logic
...
The `@gmloop/format` workspace *should* handle:
...
- `@gmloop/core`: Pure data model + shared utilities, AST types/interfaces, traversal helpers used by the format and parser to walk the AST.
- `@gmloop/parser`: GML → AST only
- `@gmloop/format`: AST → AST → GML

### The Litmus Test

This single question tells you if the architecture is clean:

>“If a new parser produced the same AST, could we swap it in without changing the formatter?”
...
````ts
// In another workspace that depends on @gmloop/core:
