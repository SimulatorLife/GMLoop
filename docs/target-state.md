...
Refactor and codemod flows must consume resolved semantic occurrences with complete certainty. Before producing edits, the refactor engine should know exactly what each affected reference is, where it is declared, where it is used, what owns it, what it depends on, what depends on it, how it is written syntactically, whether it is receiver-dependent or inherited, and whether it conflicts with a built-in or reserved identifier.

The entire project must be semantically analyzed before project-wide refactors run. Unresolved, ambiguous, shadowed, same-name, receiver-dependent, bare-call, inherited, dynamic, macro-related, or cross-scope references are not a reason to guess, silently skip, or apply partial edits. They indicate a gap in the semantic, refactor, parsing, bridge, or project-index services layer that must be resolved or surfaced as an explicit semantic-gap failure.

No refactor should rely on syntax-only heuristics; semantic certainty is required. No partial rename should be allowed to leave mixed old/new names. The long-term standard is that GMLoop understands the whole GameMaker project so that refactor/codemod behavior is deterministic, complete, and explainable across all possible GameMaker scopes.

Unresolved bare-calls and property accesses should explicitly block project-wide rename operations (yielding a non-zero exit code and diagnostic errors) instead of silently skipping them, preventing partial edits/mixed naming outcomes. But, ultimately, the goal is for NO refactors to ever fail; the semantic layer should be complete enough to resolve all references and allow safe renames in all cases. Until then, any/all failures should be diagnosed, captured, and used to improve the semantic layer with the goal of eventually eliminating all semantic gaps.
