...
- **Lint (`gml/normalize-doc-comment-tags`)** owns standalone legacy doc-comment marker and focused tag alias normalization, including `// @tag` to `/// @tag`, `// / Summary` to `/// Summary`, `//// @tag` to escaped `/// / @tag`, `@arg`/`@argument`/`@params` to `@param`, `@return` to `@returns`, `@desc` to `@description`, and visibility/exception aliases such as `@private` to `@ignore`. It does not rewrite legacy function marker aliases.
