...
This document synthesizes the target, "north-start" state for the GameMaker Language development toolkit "GMLoop", covering its formatter/linter split, semantic analysis, project-wide codemod executions, bounded-memory streaming, transpilation, hot-reload infrastructure, and more.
...
5. **Bounded-Memory Refactors**: Run large-project semantic indexing and codemod pipelines without retaining monolithic project-wide aggregates in memory. The target architecture uses bounded-memory streaming and whole-plan validation only where correctness requires it.
...
3. Keep the architecture deterministic at the output level while allowing internal processing-order differences.
4. Eliminate monolithic in-memory identifier accumulation.
5. Prevent unbounded edit and content-overlay growth during codemods.
6. Refactor-codemods correctness:
  - Must have a full, correct, whole-project rename plan.
  - Circular rename detection.
  - Duplicate target-name collision detection.
  - Cross-file consistency preflight checks.
  - Final transformed GML content must remain semantically equivalent.
