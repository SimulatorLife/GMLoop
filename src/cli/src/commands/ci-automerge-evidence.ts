...
function collectFiles(rootDirectory: string): Array<string> {
    if (!pathExistsSync(rootDirectory)) return [];
...
function computeFingerprint(): string {
    ...
    const addFile = (file: string): void => {
        if (pathExistsSync(file, (stat) => stat.isFile())) candidates.add(normalizePath(file));
    };
...
function createTestWeights(tests: ReadonlyArray<string>, historyFile: string | undefined): Array<TestWeight> {
    ...
    for (const testFile of tests) sizes.set(testFile, pathExistsSync(testFile) ? fs.statSync(testFile).size : 1);
...
function commandRecordShard(args: ParsedArgs): number {
    ...
    let xml = pathExistsSync(reportPath) ? fs.readFileSync(reportPath, "utf8") : "";
    let junitComplete =
...
function commandAssemble(args: ParsedArgs): number {
    ...
    for (const shard of manifest.shards) {
        ...
        const xml = pathExistsSync(xmlPath) ? fs.readFileSync(xmlPath, "utf8") : "";
...
function selfTest(): void {
    ...
    // Regression check for the `fs.existsSync` -> `pathExistsSync` migration:
    // `collectFiles` must still return an empty list for a directory that was
    // never created, and must still discover files once one exists, matching
    // the historical `fs.existsSync` contract this file relied on.
    const missingRoot = path.join(os.tmpdir(), `gml-ci-automerge-evidence-self-test-missing-${process.pid}`);
    assert.deepEqual(collectFiles(missingRoot), []);
    const scratchRoot = fs.mkdtempSync(path.join(os.tmpdir(), "gml-ci-automerge-evidence-self-test-"));
    try {
        fs.writeFileSync(path.join(scratchRoot, "probe.txt"), "x", "utf8");
        const relativeProbe = normalizePath(path.relative(process.cwd(), path.join(scratchRoot, "probe.txt")));
        assert.deepEqual(collectFiles(scratchRoot), [relativeProbe]);
    } finally {
