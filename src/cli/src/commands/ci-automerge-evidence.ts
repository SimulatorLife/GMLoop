...
const GITHUB_OUTPUT_OPTION = "github-output";
const REPORT_DIR_OPTION = "report-dir";
...
function commandFingerprint(args: ParsedArgs): number {
    ...
    appendOutputs(args.options[GITHUB_OUTPUT_OPTION], { fingerprint });
    return 0;
}
...
function commandManifest(args: ParsedArgs): number {
    ...
    appendOutputs(args.options[GITHUB_OUTPUT_OPTION], {
...
function commandRecordBuild(args: ParsedArgs): number {
    ...
    writeJson(path.join(requireOption(args, REPORT_DIR_OPTION), BUILD_FILE), evidence);
    appendOutputs(args.options[GITHUB_OUTPUT_OPTION], { completed, succeeded, status, timed_out: timedOut });
    return completed ? 0 : 2;
}

function commandRecordLint(args: ParsedArgs): number {
    const reportDirectory = requireOption(args, REPORT_DIR_OPTION);
    const status = parseInteger(requireOption(args, "status"), 255);
    const timedOut = parseBoolean(args.options["timed-out"]);
    const durationMs = parseInteger(args.options["duration-ms"]);
...
function commandValidate(args: ParsedArgs): number {
    const result = validateEvidenceDirectory(
        requireOption(args, REPORT_DIR_OPTION),
    ...
    appendOutputs(args.options[GITHUB_OUTPUT_OPTION], { valid: result.valid, kind: result.kind });
