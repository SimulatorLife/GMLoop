...
const EVIDENCE_FILES = [
    "src/cli/src/commands/ci-automerge-evidence.ts", "src/cli/src/commands/ci-automerge-gate.ts",
    "src/cli/src/commands/ci-automerge-state.ts", ".github/ci/automerge-policy.json",
    ".github/actions/run-automerge-validation/action.yml", ".github/workflows/automerge-prs.yml",
    ".github/workflows/cache-automerge-base-report.yml", "package.json", "pnpm-lock.yaml", "pnpm-workspace.yaml",
    ".nvmrc", ".npmrc", ".pnpmfile.cjs", "eslint.config.js"
] as const;
...
type TestManifest = Readonly<{ schemaVersion: number; tests: ReadonlyArray<string>; shardCount: number;
    manifestDigest: string; planDigest: string; shards: ReadonlyArray<TestShard> }>;
type BuildEvidence = Readonly<{ schemaVersion: number; targetSha: string; command: string; completed: boolean;
    succeeded: boolean; status: number; timedOut: boolean; durationMs: number;
    testsSkippedReason: "build-failed" | "infrastructure" | null }>;
type ShardEvidence = Readonly<{ schemaVersion: number; shard: string; completed: boolean; status: number;
    timedOut: boolean; durationMs: number; testFiles: ReadonlyArray<string>; manifestDigest: string;
    planDigest: string; reportFile: string }>;
type TestCaseEvidence = Readonly<{ file: string; name: string; status: "passed" | "failed" | "skipped";
    durationMs: number; shard: string }>;
...
function computeFingerprint(): string {
    ...
    for (const file of collectFiles(path.join(process.cwd(), "src"))) {
        if (/(?:^|\/)package\.json$/u.test(file) || /(?:^|\/)tsconfig(?:\.[^/]+)?\.json$/u.test(file)) candidates.add(file);
    }
...
function discoverCanonicalTests(): Array<string> {
    const discovered = [...collectFiles(path.join(process.cwd(), "src")), ...collectFiles(path.join(process.cwd(), "test", "dist"))];
...
function median(values: ReadonlyArray<number>): number {
    ...
    return sorted.length % 2 === 1 ? (sorted[midpoint] ?? 1000) : ((sorted[midpoint - 1] ?? 1000) + (sorted[midpoint] ?? 1000)) / 2;
}
...
function createTestWeights(tests: ReadonlyArray<string>, historyFile: string | undefined): Array<TestWeight> {
    ...
    const history = isRecord(historyValue) && Array.isArray(historyValue.fileDurations) ? historyValue.fileDurations : [];
    const durations = new Map<string, number>();
    for (const entry of history) {
        if (!isRecord(entry) || typeof entry.file !== "string" || typeof entry.durationMs !== "number" || entry.durationMs <= 0) continue;
        durations.set(normalizePath(entry.file), entry.durationMs);
    }
    const known = tests.map((testFile) => durations.get(testFile)).filter((value): value is number => value !== undefined);
...
function createBalancedShards(weights: ReadonlyArray<TestWeight>, shardCount: number): Array<TestShard> {
    if (!Number.isInteger(shardCount) || shardCount < 1 || weights.length < shardCount) throw new Error("Invalid non-empty shard plan request.");
    const shards = Array.from({ length: shardCount }, (_, index) => ({ name: `shard-${index + 1}`, files: [] as Array<string>, estimatedDurationMs: 0 }));
    const ordered = [...weights].sort((left, right) => right.weightMs - left.weightMs || left.file.localeCompare(right.file));
    for (const weight of ordered) {
        const target = [...shards].sort((left, right) => left.estimatedDurationMs - right.estimatedDurationMs || left.name.localeCompare(right.name))[0];
    ...
    return shards.map((shard) => Object.freeze({
        name: shard.name,
...
function calculatePlanDigest(shards: ReadonlyArray<TestShard>): string {
    return digestText([...shards].sort((left, right) => left.name.localeCompare(right.name))
        .flatMap((shard) => shard.files.map((file) => `${shard.name}:${file}`)).join("\0"));
}

function parseManifest(value: unknown): TestManifest {
    if (!isRecord(value) || value.schemaVersion !== MANIFEST_SCHEMA_VERSION || !Array.isArray(value.tests) || !Array.isArray(value.shards)) {
    ...
    const shardCount = typeof value.shardCount === "number" && Number.isInteger(value.shardCount) ? value.shardCount : 0;
    const shards: Array<TestShard> = [];
    for (const rawShard of value.shards) {
        if (!isRecord(rawShard) || typeof rawShard.name !== "string" || !Array.isArray(rawShard.files)) throw new Error("Invalid test shard.");
        const files = rawShard.files.filter((entry): entry is string => typeof entry === "string").map(normalizePath);
        if (files.length === 0 || files.length !== rawShard.files.length) throw new Error(`Invalid files for shard ${rawShard.name}.`);
    ...
    if (manifest.shardCount !== manifest.shards.length || manifest.tests.length === 0) throw new Error("Manifest shard/test count is invalid.");
    if (manifest.manifestDigest !== calculateManifestDigest(manifest.tests) || manifest.planDigest !== calculatePlanDigest(manifest.shards)) throw new Error("Manifest digest is invalid.");
    const assigned = manifest.shards.flatMap((shard) => shard.files);
    if (new Set(assigned).size !== assigned.length || JSON.stringify([...assigned].sort()) !== JSON.stringify([...manifest.tests].sort())) {
        throw new Error("Manifest does not cover every canonical test exactly once.");
    }
    if (!manifest.tests.includes("test/dist/fixture-suites.js")) throw new Error("Fixture-suite runner is missing from the canonical corpus.");
    return manifest;
}
...
function commandManifest(args: ParsedArgs): number {
    ...
    if (tests.length === 0 || !tests.includes("test/dist/fixture-suites.js")) throw new Error("Canonical test corpus is incomplete.");
    ...
    appendOutputs(args.options["github-output"], { matrix: JSON.stringify({ shard: shards.map((shard) => shard.name) }) });
    return 0;
}
...
function commandStageCompiled(args: ParsedArgs): number {
    ...
    const files = [...collectFiles(path.join(process.cwd(), "src")), ...collectFiles(path.join(process.cwd(), "test", "dist"))]
        .filter((file) => (file.startsWith("src/") && file.includes("/dist/")) || file.startsWith("test/dist/"));
    for (const file of files) {
        if (file.endsWith(".map") || file.endsWith(".d.ts") || file.endsWith(".d.ts.map") || file.endsWith(".tsbuildinfo")) continue;
...
function commandRecordLint(args: ParsedArgs): number {
    ...
    writeJson(path.join(reportDirectory, LINT_META_FILE), { schemaVersion: SCHEMA_VERSION, completed, status, timedOut, durationMs });
    return completed ? 0 : 2;
}
...
function commandRecordShard(args: ParsedArgs): number {
    ...
    let junitComplete = xml.includes("<testsuites") && xml.includes("</testsuites>") && !xml.includes(INFRA_JUNIT_MARKER);
    if (!junitComplete && (timedOut || status > 1)) {
        const reason = timedOut ? "Shard wall-clock watchdog expired." : `Node test runner ended abnormally with status ${status}.`;
...
function validateBuild(value: unknown, expectedSha: string): Readonly<{ valid: boolean; succeeded: boolean; errors: ReadonlyArray<string> }> {
...
function commandAssemble(args: ParsedArgs): number {
    ...
    let complete = build.valid && build.succeeded && isRecord(lintMeta) && lintMeta.completed === true && [0, 1].includes(Number(lintMeta.status));
    ...
    for (const shard of manifest.shards) {
        ...
        const assignedFiles = Array.isArray(metadata.testFiles) ? metadata.testFiles.filter((entry): entry is string => typeof entry === "string") : [];
        const validShard = metadata.schemaVersion === SCHEMA_VERSION && metadata.shard === shard.name && metadata.completed === true
            && [0, 1].includes(Number(metadata.status)) && metadata.manifestDigest === manifest.manifestDigest
            && metadata.planDigest === manifest.planDigest && JSON.stringify([...assignedFiles].sort()) === JSON.stringify([...shard.files].sort())
        ...
        for (const file of shard.files) durations.set(file, Math.max(1, Math.round(attributed.get(file) ?? fallback ?? 1000)));
        shardRows.push({ name: shard.name, completed: metadata.completed === true, status: Number(metadata.status ?? 2), timedOut: metadata.timedOut === true, durationMs: shardDuration, testFileCount: shard.files.length });
    }
    const report = {
        schemaVersion: SCHEMA_VERSION,
        completed: complete,
...
function validateEvidenceDirectory(reportDirectory: string, expectedSha: string, expectedFingerprint: string): Readonly<{ valid: boolean; kind: "full" | "build-failure" | "invalid"; errors: ReadonlyArray<string> }> {
    ...
    try { manifest = parseManifest(readJson(path.join(reportDirectory, MANIFEST_FILE))); } catch (error) { errors.push(error instanceof Error ? error.message : String(error)); }
    if (!isRecord(report) || report.schemaVersion !== SCHEMA_VERSION || report.completed !== true || report.targetSha !== expectedSha) errors.push("full report is incomplete or targets the wrong SHA");
    if (isRecord(report) && report.toolingFingerprint !== expectedFingerprint) errors.push("trusted tooling fingerprint mismatch");
    if (!Array.isArray(lint) || !Array.isArray(cases)) errors.push("lint or normalized test-case evidence is missing");
    if (manifest && isRecord(report) && (report.manifestDigest !== manifest.manifestDigest || report.planDigest !== manifest.planDigest)) errors.push("report/manifest digest mismatch");
    return Object.freeze({ valid: errors.length === 0, kind: errors.length === 0 ? "full" : "invalid", errors: Object.freeze(errors) });
}

function commandValidate(args: ParsedArgs): number {
    const result = validateEvidenceDirectory(requireOption(args, "report-dir"), requireOption(args, "expected-sha"), requireOption(args, "expected-fingerprint"));
...
function commandPromote(args: ParsedArgs): number {
    ...
    if (!promoted.valid || promoted.kind !== "full") throw new Error(`Promoted evidence is invalid: ${promoted.errors.join("; ")}`);
    return 0;
}

function selfTest(): void {
    const tricky = '<?xml version="1.0"?><testsuites><testsuite>'
    ...
    assert.deepEqual(parsed.map((entry) => entry.status), ["passed", "skipped", "failed"]);
    const shards = createBalancedShards([{ file: "a", weightMs: 20 }, { file: "b", weightMs: 10 }], 2);
