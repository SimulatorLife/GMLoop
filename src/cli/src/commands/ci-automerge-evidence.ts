...
function normalizeRepositoryPath(value: string): string {
    const normalized = Core.toPosixPath(value);
    const root = Core.toPosixPath(process.cwd()).replace(/\/$/u, "");
...
function collectFiles(rootDirectory: string): Array<string> {
    ...
    const visit = (directory: string): void => {
        ...
        for (const entry of entries) {
            ...
            else if (entry.isFile()) output.push(Core.toPosixPath(path.relative(process.cwd(), absolutePath)));
        }
    };
    visit(rootDirectory);
    return output;
...
function computeFingerprint(): string {
    ...
    const addFile = (file: string): void => {
        if (fs.existsSync(file) && fs.statSync(file).isFile()) candidates.add(Core.toPosixPath(file));
    };
...
function createTestWeights(tests: ReadonlyArray<string>, historyFile: string | undefined): Array<TestWeight> {
    ...
    for (const entry of history) {
        if (
            ...
            continue;
        durations.set(Core.toPosixPath(entry.file), entry.durationMs);
    }
    const known = tests
