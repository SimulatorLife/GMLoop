...
const EXCLUDED_DIRECTORY_NAMES = new Set(Core.DEFAULT_PROJECT_EXCLUDES.directoryNames);
 ...
 *
 * Delegates tree traversal to the shared {@link traverseDirectoryEntries}
 * helper so this function is responsible only for project-wide exclusion and
 * the `.gml` file filter — no queue bookkeeping lives inline here.
 *
...
function collectGmlFilesFromDirectory(directoryPath: string): Array<string> {
    ...
    traverseDirectoryEntries(directoryPath, {
        shouldDescend: (_fullPath, entry) => !EXCLUDED_DIRECTORY_NAMES.has(entry.name),
        onFile: (filePath, entry) => {
            if (entry.name.toLowerCase().endsWith(GML_FILE_EXTENSION)) {
                discoveredFilePaths.push(filePath);
            }
        },
