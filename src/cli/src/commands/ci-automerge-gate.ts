...
function selfTest(): void {
    const lintTestFile = "/repo/a.ts";
    const baselineLint = [
        { filePath: lintTestFile, messages: [{ severity: 1, ruleId: "x", message: "old", line: 1 }] }
    ];
    const movedLint = [{ filePath: lintTestFile, messages: [{ severity: 1, ruleId: "x", message: "old", line: 99 }] }];
    assert.equal(compareLint(baselineLint, movedLint).added.length, 0);
    const upgradedLint = [
        { filePath: lintTestFile, messages: [{ severity: 2, ruleId: "x", message: "old", line: 99 }] }
    ];
    assert.equal(compareLint(baselineLint, upgradedLint).added.length, 1);
    const baselineMaxLines = [
        {
            filePath: lintTestFile,
            messages: [
