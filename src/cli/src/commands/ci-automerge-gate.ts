...
const BUILD_FAILURE_KIND = "build-failure";
...
function readCases(value: unknown): Array<CaseEvidence> {
    ...
    return value.map((entry) => {
        if (
            ...
            !["passed", "failed", "skipped"].includes(entry.status as string)
        )
...
function evidenceKind(directory: string): "full" | "build-failure" {
    ...
    if (build.succeeded !== true) return BUILD_FAILURE_KIND;
    ...
    if (mergeKind === BUILD_FAILURE_KIND) {
        ...
        if (baselineDirectory) {
            ...
            if (hasRegression) {
                ...
                green = true;
                reason = baseKind === BUILD_FAILURE_KIND ? "recovery" : "clean";
                lines.push(
                    "",
