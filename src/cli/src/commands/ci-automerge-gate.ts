...
function collectLintFindings(value: unknown): Array<LintFinding> {
    ...
    for (const fileValue of value) {
        ...
        for (const message of fileValue.messages) {
            ...
            const normalizedMessage = normalizeLintMessage(ruleId, message.message);
            const identity = [file, ruleId, normalizedMessage].join("\0");
            const relocationIdentity = [path.posix.basename(file), ruleId, normalizedMessage].join("\0");
            output.push(Object.freeze({ file, severity, ruleId, message: message.message, identity, relocationIdentity }));
        }
    }
    return output;
}

function addFindingToMap(
    map: Map<string, Array<LintFinding>>,
    key: string,
    finding: LintFinding
): void {
...
function comparableSeverities(findings: ReadonlyArray<LintFinding>): Array<number> {
    return findings.map((finding) => finding.severity).toSorted((left, right) => right - left);
}

function uniqueFindingPaths(findings: ReadonlyArray<LintFinding>): Set<string> {
    return new Set(findings.map((finding) => finding.file));
}
...
function compareLint(baseValue: unknown, targetValue: unknown): Readonly<{ baseCount: number; targetCount: number; added: ReadonlyArray<LintFinding> }> {
    ...
    const baseByIdentity = new Map<string, Array<LintFinding>>();
    const baseByRelocationIdentity = new Map<string, Array<LintFinding>>();
    const targetByIdentity = new Map<string, Array<LintFinding>>();
    const targetByRelocationIdentity = new Map<string, Array<LintFinding>>();
    for (const finding of base) {
        addFindingToMap(baseByIdentity, finding.identity, finding);
        addFindingToMap(baseByRelocationIdentity, finding.relocationIdentity, finding);
    }
    for (const finding of target) {
        addFindingToMap(targetByIdentity, finding.identity, finding);
        addFindingToMap(targetByRelocationIdentity, finding.relocationIdentity, finding);
    }
    const added: Array<LintFinding> = [];
    for (const [identity, findings] of targetByIdentity) {
        const exactBaseline = baseByIdentity.get(identity);
        let baselineSeverities = exactBaseline ? comparableSeverities(exactBaseline) : [];
        if (!exactBaseline) {
            const sample = findings[0];
            const relocatedBaseline = sample ? baseByRelocationIdentity.get(sample.relocationIdentity) : undefined;
            const relocatedTarget = sample ? targetByRelocationIdentity.get(sample.relocationIdentity) : undefined;
            if (relocatedBaseline && relocatedTarget
                && uniqueFindingPaths(relocatedBaseline).size === 1
                && uniqueFindingPaths(relocatedTarget).size === 1) {
                baselineSeverities = comparableSeverities(relocatedBaseline);
            }
        }
        const sortedFindings = [...findings].toSorted((left, right) => right.severity - left.severity);
...
function selfTest(): void {
    ...
    const relocatedLint = [{ filePath: "/repo/nested/a.ts", messages: [{ severity: 1, ruleId: "x", message: "old", line: 99 }] }];
    assert.equal(compareLint(baselineLint, relocatedLint).added.length, 0);
    const renamedLint = [{ filePath: "/repo/nested/b.ts", messages: [{ severity: 1, ruleId: "x", message: "old", line: 99 }] }];
    assert.equal(compareLint(baselineLint, renamedLint).added.length, 1);
