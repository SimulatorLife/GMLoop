...
- It seems odd that the "refactor" workspace does not seem to be dependent on "semantic"? Is that the expected/desired state/behavior? Why or why not? Does it not need the semantic-backed analysis (variable scopes, dependencies/dependents, etc.) to be able to make proper/clean refactors? Why or why not? What is the expected/desired/target interaction/boundaries across refactor + semantic?

    This is based on looking at "src/refactor/package.json" which only has:
    ```
    "dependencies": {
