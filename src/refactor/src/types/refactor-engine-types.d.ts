...
import type { ConflictSeverityValue, ConflictTypeValue, GlobalvarToGlobalCodemodOptions, MaybePromise, NamingCategory, Range, RefactorCodemodConfigMap, RefactorCodemodId, RefactorProjectConfig } from "../types.js";
...
export interface ConfiguredCodemodRunRequest {
    ...
    onProgress?: (progress: { current: number; total: number; filePath: string }) => MaybePromise<void>;
    onAfterCodemod?: (summary: ConfiguredCodemodSummary, context: {
        readFile: WorkspaceReadFile;
    }) => MaybePromise<void>;
}
/**
...
export interface FeatherRenamePlanner {
    planFeatherRenames(requests: ReadonlyArray<{
        identifierName: string;
        preferredReplacementName: string;
    }>, filePath: string | null, projectRoot: string, context: RefactorProjectAnalysisContext): Promise<Array<{
        identifierName: string;
...
export interface GlobalVarRewriteAssessor {
    assessGlobalVarRewrite(filePath: string | null, hasInitializer: boolean): {
 ...
 */
export interface RefactorProjectAnalysisProvider extends IdentifierOccupancyChecker, FeatherRenamePlanner, GlobalVarRewriteAssessor, LoopHoistIdentifierResolver {
}
/**
...
export interface RefactorHotReloadCoordinator {
    checkHotReloadSafety(request: RenameRequest, semantic: PartialSemanticAnalyzer | null): Promise<HotReloadSafetySummary>;
    computeHotReloadCascade(changedSymbolIds: Array<string>, semantic: PartialSemanticAnalyzer | null): Promise<HotReloadCascadeResult>;
...
export interface CodemodTransformExecutor {
    executeGlobalvarToGlobalCodemod(request: ExecuteGlobalvarToGlobalCodemodRequest): Promise<ExecuteGlobalvarToGlobalCodemodResult>;
}
/**
...
export interface CodemodRenameOperations {
    validateRenameRequest(request: RenameRequest, options?: ValidateRenameRequestOptions): Promise<ValidationSummary & {
        symbolName?: string;
        occurrenceCount?: number;
        hotReload?: HotReloadSafetySummary;
    }>;
    prepareBatchRenamePlan(request: Array<RenameRequest>, options?: PrepareBatchRenamePlanOptions): Promise<BatchRenamePlanSummary>;
 ...
 */
export interface CodemodEngine extends CodemodSemanticProvider, CodemodTransformExecutor, CodemodRenameOperations, CodemodWorkspaceEditor, CodemodCacheController {
}
...
//# sourceMappingURL=refactor-engine-types.d.ts.map