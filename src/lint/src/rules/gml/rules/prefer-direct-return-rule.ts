...
function buildDirectReturnCandidate(
    ...
    if (!isReturnStatementNode(nextStatement)) {
        return null;
    }

    // The return argument may be wrapped in `ParenthesizedExpression` nodes
    // when the author wrote `return (var)` or `return ((var))`; the GML
    // parser preserves those synthetic parentheses so we must unwrap before
    // checking for a plain identifier match.
    const unwrappedReturnArgument = Core.unwrapParenthesizedExpression(nextStatement.argument);
    if (!isIdentifierNode(unwrappedReturnArgument)) {
        return null;
    }

    if (unwrappedReturnArgument.name !== declarator.id.name) {
        return null;
    }
