...
Semantic facts, whether durable or session-local, are acquired through `SemanticIndexStore.acquireSemanticSnapshot`, not by treating a cache row as an unqualified project index. A lease has an exact revision, generation, tier, capability set, coverage set, validation state, and overlay version map. Callers must release it once they have finished deriving their request-local view.

- `definitions` supplies interactive declaration capabilities: completion, hover, definition, symbols, and semantic tokens.
- `full` additionally supplies diagnostics, references, and rename-safety facts, and is only leaseable when it matches the active definitions revision.
...
- Snapshot-backed refactor queries expose symbols, exact occurrences, file definitions, and rename-safety gaps without rebuilding a navigation projection or interpreting a raw project index.
