...
/**
 * Default set of scope kinds that represent an `other` instance context (e.g.
 * a `with (other) { ... }` block). Defaults to an empty set because no
 * canonical scope kind string is yet registered for such blocks; callers
 * that introduce one must opt in by passing a custom set. Identifiers that
 * cannot be resolved inside an opted-in scope are classified as `other_field`
 * so the transpiler emits `other.<name>` rather than `self.<name>` or a bare
 * identifier.
 */
const DEFAULT_OTHER_CONTEXT_SCOPE_KINDS: ReadonlySet<string> = new Set();
...
export class BasicSemanticOracle implements IdentifierAnalyzer, CallTargetAnalyzer {
    ...
    private readonly otherContextScopeKinds: ReadonlySet<string>;
     ...
     * @param otherContextScopeKinds Set of scope kinds that indicate an
     *                               `other` instance context (e.g. the body of
     *                               a `with (other) { ... }` block). When an
     *                               identifier cannot be resolved in the scope
     *                               chain and the current scope stack contains
     *                               one of these kinds, the identifier is
     *                               classified as `other_field` so the
     *                               transpiler emits `other.<name>`. Checked
     *                               before `self_field` because a nested
     *                               `with (other)` rebinds the implicit
     *                               instance reference. Defaults to an empty
     *                               set; callers must opt in by passing a set
     *                               containing their chosen scope kind string.
     */
    constructor(
        ...
        otherContextScopeKinds: ReadonlySet<string> = DEFAULT_OTHER_CONTEXT_SCOPE_KINDS
    ) {
        this.tracker = tracker;
        this.builtinNames = builtinNames;
        this.scriptNames = scriptNames;
        this.selfContextScopeKinds = selfContextScopeKinds;
        this.otherContextScopeKinds = otherContextScopeKinds;
    }
    ...
    /**
     * Returns true when the current scope stack contains at least one scope
     * whose kind is listed in `otherContextScopeKinds`. This indicates that
     * unresolved identifiers should be treated as `other` instance fields
     * (e.g. bare references inside a `with (other) { ... }` block). Mirrors
     * `isInSelfContext` but is checked first so a nested `with (other)`
     * rebinds the implicit instance reference.
     */
    private isInOtherContext(): boolean {
        if (!this.tracker) {
            return false;
        }
        const stack = this.tracker.getScopeStack();
        for (let i = stack.length - 1; i >= 0; i--) {
            if (this.otherContextScopeKinds.has(stack[i].kind)) {
                return true;
            }
        }
        return false;
     ...
     * 5. Unresolved identifiers inside an `other` context scope → `other_field`
     * 6. Unresolved identifiers inside an object-event scope → `self_field`
     * 7. Default to "local" for all other unresolved identifiers
     *
     * Step 5 handles the `with (other) { ... }` pattern where the body refers
     * to fields of the `other` instance by bare name. Because those fields
     * are never declared as locals, the scope-chain lookup fails and the
     * oracle falls back to `other_field`, directing the transpiler to emit
     * `other.<name>` instead of a bare identifier. This step is checked
     * before `self_field` because a nested `with (other)` rebinds the
     * implicit instance reference; an `object_event` ancestor must not
     * shadow a closer `other` context.
     *
     * Step 6 handles the common GML pattern where an object event accesses an
     ...
     *
     * Both step 5 and step 6 are configurable: the caller passes the set of
     * scope kinds that should trigger each classification via the
     * `otherContextScopeKinds` and `selfContextScopeKinds` constructor
     * parameters. The default `otherContextScopeKinds` is empty because no
     * canonical scope kind string is yet registered for `with (other)`
     * blocks; callers must opt in. The `other` keyword in GML is also
     * syntactically represented as a member expression (`other.x`) by the
     * parser, so explicit `other.x` accesses are handled structurally by
     * the transpiler regardless of this classification.
     */
    kindOfIdent(node: IdentifierMetadata | null | undefined): SemKind {
        if (!node?.name) {
            return "local";
        ...
        if (this.tracker) {
            ...
            // `other` is checked before `self` because a nested `with (other) { ... }`
            // rebinds the implicit instance inside the enclosing event scope.
            if (this.isInOtherContext()) {
                return "other_field";
            }
