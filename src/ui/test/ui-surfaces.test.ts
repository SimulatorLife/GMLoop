...
void test("UI surface catalog defines implemented and planned cross-workspace views", () => {
    ...
    const autoGameSurface = UI.getUISurfaceDefinition("auto-game");
    ...
    assert.equal(autoGameSurface.status, "implemented");
    ...
    assert.deepEqual(
        ...
        ["graph", "ast", "config", "docs", "fix", "live-reload", "auto-game", "playground", "rules"]
    );
});
