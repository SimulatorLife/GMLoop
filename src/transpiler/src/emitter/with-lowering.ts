...
export function lowerWithStatement(testExpression: string, rawBody: string, resolveWithTargetsIdent: string): string {
    ...
    const lines = [
        ...
        "            // Snapshot the target list so mutations performed by the loop body",
        "            // (destroying/creating instances that push/splice this same array)",
        "            // cannot shrink or grow __with_targets out from under the index-based",
        "            // loop below, which would otherwise skip or re-visit entries.",
        "            return [...__with_value];",
        "        }",
        "        return [__with_value];",
        "    })();",
